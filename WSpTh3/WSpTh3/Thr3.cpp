...
LONG WndProc_OnCreate(HWND hWnd, LPCREATESTRUCT lpCreateStruct)
{
	...
	bSuspendR = false;
	//Thread PaintX starting not suspended!
	bSuspendX = false;
...
unsigned int __stdcall  PaintX(void *hWnd) {
	////////void PaintX(void *hWnd){

	HDC hDC;
	RECT rect;
	LONG xLeft, xRight, yTop, yBottom, xr1, yr1, xr2, yr2;
	short nRed, nGreen, nBlue;
	HBRUSH hBrush, hOldBrush;
	DWORD dwRetCode;
	...
	while (!fTerminateX) {
		...
		case WAIT_OBJECT_0:
		{
			...
			xr1 = rand() % (rect.left + 1);
			yr1 = rand() % (rect.right + 1);
			xr2 = rand() % (rect.top + 1);
			yr2 = rand() % (rect.bottom + 1);

			hBrush = CreateSolidBrush(RGB(nRed, nGreen, nBlue));
			hOldBrush = (HBRUSH)SelectObject(hDC, hBrush);
			Pie(hDC, min(xLeft, xRight), min(yTop, yBottom),
				max(xLeft, xRight), max(yTop, yBottom),
				xr1, yr1, xr2, yr2);
			SelectObject(hDC, hOldBrush);
			DeleteObject(hBrush);
